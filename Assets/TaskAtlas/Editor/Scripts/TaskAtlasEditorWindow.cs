...
namespace TaskAtlasNamespace
{
    ...
    public class TaskAtlasEditorWindow : EditorWindow
    {
        ...
        public static TaskAtlasData.Scene scene;
        ...
        {
            if (Core.data == null) Core.Init();
            if (tags == null) tags = new List<string>();
            if (tagList == null) { tagList = new List<string>(); } else { tagList.Clear(); }
            tagList.Add("Show All");
            for (int i = 0; i < Core.data.tags.Count; i++)
            {
                tagList.Add(Core.data.tags[i].name);
            }
        ...
        static public void Init()
        {
            ...
            scene = Core.data.scene[Core.data.sceneIndex];
            RefreshSettings();
            scene.UpdateAllProgress();
        ...
        static bool LoadViewLandmarkButtons(int i)
        {
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                GUILayout.BeginVertical();
                {
                    ...
                    if (scene.landmarks[i].viewState == TaskAtlasData.Scene.Landmark.ViewState.is2D)
                        DrawTexture(new Rect(pos.x + pos.width - 50, pos.y + pos.height - 26, 48, 24), Core.tIcon2D);
                    else if (scene.landmarks[i].viewState == TaskAtlasData.Scene.Landmark.ViewState.is3D)
                        DrawTexture(new Rect(pos.x + pos.width - 50, pos.y + pos.height - 26, 48, 24), Core.tIcon3D);
                    else if (scene.landmarks[i].viewState == TaskAtlasData.Scene.Landmark.ViewState.isOrthographic)
                    ...
                    GUILayout.BeginVertical(sBackground);
                    {
                        ...
                        GUILayout.BeginHorizontal(GUILayout.Height(bSize * 1.2f));
                        {
                            ...
                            if (GUILayout.Button(Core.tEditPen, GUILayout.Width(bSize), GUILayout.Height(bSize)))
                            {
                                ...
                                scene.landmarkDetailState = TaskAtlasData.Scene.LandmarkDetailState.general;
                            }
                    ...
                    GUILayout.BeginVertical(sBackground);
                    {
                        ...
                        GUILayout.BeginHorizontal(GUILayout.Height(bSize * 1.2f));
                        {
                            ...
                            if (GUILayout.Button(Core.tIconTask, GUILayout.Width(bSize), GUILayout.Height(bSize)))
                            {
                                scene.selectedLandmark = i;
                                scene.landmarkDetailOpen = true;
                                scene.landmarkDetailState = TaskAtlasData.Scene.LandmarkDetailState.tasks;
                            }
                    ...
                    GUILayout.BeginVertical(sBackground);
                    {
                        ...
                        GUILayout.BeginHorizontal(GUILayout.Height(bSize * 1.2f));
                        {
                            ...
                            if (GUILayout.Button(Core.tGallery, GUILayout.Width(bSize), GUILayout.Height(bSize)))
                            {
                                scene.selectedLandmark = i;
                                scene.landmarkDetailOpen = true;
                                scene.landmarkDetailState = TaskAtlasData.Scene.LandmarkDetailState.gallery;
                            }
        ...
        static void LoadViewLandmarkDetailButtons(int i)
        {
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                if (scene.landmarkDetailState == TaskAtlasData.Scene.LandmarkDetailState.general)
                {
                    GUILayout.BeginVertical();
                }
                else
                ...
                if (scene.landmarkDetailState == TaskAtlasData.Scene.LandmarkDetailState.tasks)
                {
                    GUILayout.BeginVertical();
                }
                else
                ...
                //             scene.selectedLandmark = i;
                //             scene.landmarkDetailOpen = true;
                //             scene.landmarkDetailState = TaskAtlasData.Scene.LandmarkDetailState.delete;
                //         }
            ...
            GUILayout.BeginArea(new Rect(screenWidthMargin, 0, screenWidth - 6, screenHeight));
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    GUILayout.BeginVertical(GUILayout.ExpandHeight(true));
                    {
                        ...
                        {
                            #region GeneralState
                            case TaskAtlasData.Scene.LandmarkDetailState.general:
                                LoadViewLandmarkDetailGeneral(scene.selectedLandmark);
                                break;
                            #endregion

                            #region GalleryState
                            case TaskAtlasData.Scene.LandmarkDetailState.gallery:
                                ...
                                //     {
                                //         localData.gallery.Add(new TaskAtlasData.Scene.Landmark.Gallery("New Image"));
                                //         Core.SaveData();
                                //     }
                                //     s = StyleCheck();
                                //     #endregion
                                ...
                                GUILayout.BeginHorizontal();
                                {
                                    ...
                                    {
                                        gallery.images.Add(new TaskAtlasData.Scene.Landmark.Gallery.Image((Texture2D)AssetDatabase.LoadAssetAtPath(Core.taPath + "/Screenshots/TaskAtlas_Default.png", typeof(Texture2D))));
                                ...
                                if (gallery.images.Count > 0)
                                {
                                    ...
                                    GUILayout.BeginHorizontal();
                                    {
                                        if (GUILayout.Button("Replace with Current View"))
                                        {
                                            ...
                                            if (gallery.images == null) gallery.images = new List<TaskAtlasData.Scene.Landmark.Gallery.Image>();
                            ...
                            #region TaskState
                            case TaskAtlasData.Scene.LandmarkDetailState.tasks:
                                GUILayout.BeginHorizontal();
                                {
                                    #region NewTask
                                    s = StyleCheck("button");
                                    ...
                                    {
                                        localData.tasks.Add(new TaskAtlasData.Scene.Landmark.Task("New Task"));
                                        Core.SaveData();
                                    }
                                    ...
                                    GUILayout.BeginVertical(StyleBack(s, Core.tGreyBright));
                                    {
                                        ...
                                        for (int i = 0; i < localData.tasks.Count; i++)
                                        {
                                            ...
                                            GUILayout.BeginHorizontal(sBackground);
                                            {
                                                ...
                                                tasks.stage = (TaskAtlasData.Scene.Landmark.Task.Stage)EditorGUILayout.EnumPopup(tasks.stage, GUILayout.Width(80));
                                                GUILayout.FlexibleSpace();
                                                ...
                                                tasks.priority = (TaskAtlasData.Scene.Landmark.Task.Priority)EditorGUILayout.EnumPopup(tasks.priority, GUILayout.Width(80));
                                            }
                                            GUILayout.EndHorizontal();
                                            #endregion
                                            string arrow;
                                            ...
                                            if (!tasks.isFolded)
                                            {
                                                ...
                                                GUILayout.BeginHorizontal(sBackground);
                                                {
                                                    ...
                                                    checksum += tasks.name;
                                                    tasks.name = EditorGUILayout.DelayedTextField(tasks.name, GUILayout.Width(120));
                                                }
                                                GUILayout.EndHorizontal();
                                                ...
                                                GUILayout.BeginHorizontal(sBackground);
                                                {
                                                    ...
                                                    tasks.description = EditorGUILayout.DelayedTextField(tasks.description, s, GUILayout.Width(screenWidth - 32), GUILayout.Height(s.fixedHeight));
                                                }
                                                GUILayout.EndHorizontal();
                                                ...
                                                if (tasks.alert)
                                                {
                                                    ...
                                                    GUILayout.BeginHorizontal(sBackground);
                                                    {
                                                        ...
                                                        tasks.month = (TaskAtlasData.Scene.Landmark.Task.Month)EditorGUILayout.EnumPopup(tasks.month);
                                                        List<int> o = new List<int>();
                                                        List<string> slist = new List<string>();
                                                        if (tasks.year == 0) tasks.year = DateTime.Now.Year;
                                                        for (int ii = 0; ii < DateTime.DaysInMonth(tasks.year, ((int)tasks.month) + 1); ii++)
                                                        {
                                                            o.Add(ii + 1);
                                                ...
                                                {
                                                    tasks.subTasks.Add(new TaskAtlasData.Scene.Landmark.Task.SubTask("New Subtask"));
                                                }
                                                ...
                                                if (tasks.isSticky)
                                                {
                                                    ...
                                                    GUILayout.BeginHorizontal(sBackground);
                                                    {
                                                        ...
                                                        if (localData.viewState != TaskAtlasData.Scene.Landmark.ViewState.is2D)
                                                        {
                                                            GUILayout.Label("Scale Factor");
                                                            GUILayout.EndHorizontal(); GUILayout.BeginHorizontal();

                                                            tasks.scale = EditorGUILayout.Slider(tasks.scale, 1, 100);

                                                            GUILayout.EndHorizontal(); GUILayout.BeginHorizontal();
                                                            s = new GUIStyle("button");

                                                            tasks.useDefaultFadeDistance = GUILayout.Toggle(tasks.useDefaultFadeDistance, "Use Landmark Fade Distance", s);
                                                            GUILayout.EndHorizontal(); GUILayout.BeginHorizontal();
                                                            if (tasks.useDefaultFadeDistance)
                                                            {
                                                                GUILayout.FlexibleSpace();
                                                                GUILayout.Label("Set Task Fade Distance in Details Tab");
                                                                GUILayout.FlexibleSpace();
                                                                GUILayout.EndHorizontal(); GUILayout.BeginHorizontal();
                                                                GUI.enabled = false;
                                ...
                                #endregion
                                break;
                            case TaskAtlasData.Scene.LandmarkDetailState.delete:
        ...
        static void DrawTagFilterMask()
        {
            ...
            if (prev != tagSelected)
            {
                ...
                for (int i = 0; i < Core.data.tags.Count; i++)
                {
                    ...
                    {
                        tags.Add(Core.data.tags[i].name);
                        Core.data.tags[i].active = true;
                    }
                    else
                    {
                        Core.data.tags[i].active = false;
                    }
                }
            }
            GUILayout.FlexibleSpace();
                    ...
                    GUILayout.BeginHorizontal();
                    {
                        ...
                        switch (tagEditMode)
                        {
                            case TagEditMode.none:
                                ...
                                {
                                    Core.data.tags.Add(new TaskAtlasData.Tags(newTag));
                                    Core.data.tags = Core.data.tags.Distinct().ToList();
                                    Core.data.tags = Core.data.tags.OrderBy(x => x.name).ToList();
                                    for (int i = 0; i < scene.landmarks.Count; i++)
                                    {
                                        scene.landmarks[i].tags.Add(new TaskAtlasData.Tags(newTag));
                                ...
                                if (GUILayout.Button(new GUIContent("  Edit"), GUILayout.Width(64), GUILayout.Height(18)))
                                {
                                    ...
                                    for (int i = 0; i < Core.data.tags.Count; i++)
                                    {
                                        tagListCopy.Add(Core.data.tags[i].name);
                                    }
                                }
                                #endregion
                                #region DeleteTags
                            ...
                            case TagEditMode.edit:
                                ...
                                if (GUILayout.Button("Save Tags", GUILayout.Width(96)))
                                {
                                    ...
                                    {
                                        if (Core.data.tags[i].name != tagListCopy[i])
                                        {
                                            Core.data.tags[i].name = tagListCopy[i];
                                        }
                                    }
                                    for (int i = 0; i < scene.landmarks.Count; i++)
                                    {
                                    ...
                                    for (int x = 0; x < Core.data.tags.Count; x++)
                                    {
                                        Core.data.tags[x].name = tagListCopy[x];
                                    }
                                    Core.data.tags = Core.data.tags.Distinct().ToList();
                                    Core.data.tags = Core.data.tags.OrderBy(x => x.name).ToList();
                                    Core.SaveData();
        ...
        static public void AddCurrentViewToLandmarks()
        {
            ...
            scene.landmarks.Add(
                new TaskAtlasData.Scene.Landmark(
                    Core.rtSS,
            ...
            scene.landmarks[scene.selectedLandmark].tags = new List<TaskAtlasData.Tags>();
            for (int i = 0; i < Core.data.tags.Count; i++)
            {
                scene.landmarks[scene.selectedLandmark].tags.Add(new TaskAtlasData.Tags(Core.data.tags[i].name));
            }
        ...
        static void RefreshLandmarkTags()
        {
            ...
            for (int i = 0; i < scene.landmarks.Count; i++)
            {
                ...
                scene.landmarks[i].tags = new List<TaskAtlasData.Tags>();

                for (int y = 0; y < Core.data.tags.Count; y++)
                {
                    scene.landmarks[i].tags.Add(new TaskAtlasData.Tags(Core.data.tags[y].name));
                    for (int x = 0; x < oldList.Count; x++)
                    {
                        if (oldList[x].name == Core.data.tags[y].name)
                        {
        ...
        static public void RefreshLandmarkThumbnail(int index)
        {
            ...
            if (scene.landmarks[index].viewState == TaskAtlasData.Scene.Landmark.ViewState.is2D)
                Core.LandmarkCamera.GetComponent<Camera>().orthographic = true;
            if (scene.landmarks[index].viewState == TaskAtlasData.Scene.Landmark.ViewState.is3D)
        ...
        static void ZoomToLocation(int i, int t = -1)
        {
            ...
            if (TaskAtlasSceneView.atlas.enabled)
            {
                ...
                {
                    sv.in2DMode = false;
                    if (scene.landmarks[i].viewState == TaskAtlasData.Scene.Landmark.ViewState.isOrthographic)
                    {
            ...
            else
            {
                ...
                sv.AlignViewToObject(Core.LandmarkCamera.transform);
                // sv.pivot = scene.landmarks[i].position + (scene.landmarks[i].position - sv.camera.transform.position);
