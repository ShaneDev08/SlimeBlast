...
namespace TaskAtlasNamespace
{
    ...
    {
        static public TaskAtlasData.Scene scene;
        ...
        public static TaskAtlasData.Scene.Atlas atlas;
        ...
        static void Init()
        {
            ...
            {
                scene.landmarks[x].UpdateProgress();
        ...
        {
            if (Core.data == null) { Core.Init(); Init(); }
            if (Core.isBuild) return;
            SetEditorDeltaTime();
        ...
        static public void RefreshSettings()
        {
            ...
            if (Core.data.tags == null) Core.data.tags = new List<TaskAtlasData.Tags>();
        ...
        static public void OnScene(SceneView sceneview)
        {
            ...
            if (scene.landmarks.Count == 0)
            {
                ...
                {
                    Core.data.sceneIndex = Core.CheckSceneExists();
                    Core.Init();
                    Init();
                    return;
                }
            ...
            for (int i = 0; i < scene.landmarks.Count; i++)
            {
                ...
                if (scene.showLandmarkLabels)
                {
                    if (!atlas.enabled)
                    {
                        if (landmarks.showGizmo)
                        {
                            ...
                            if (Event.current.type == EventType.Layout) landmarks.wsPosition = HandleUtility.WorldToGUIPointWithDepth(landmarks.position);
                            ...
                            if (!atlasCancelled)
                            {
                                if (svcFloor != lposFloor)
                                    if (sceneview.in2DMode && landmarks.viewState == TaskAtlasData.Scene.Landmark.ViewState.is2D)
                                    {
                                        if (landmarks.wsPosition.z > 0)
                                        {
                                            ...
                                            if (landmarks.wsPosition.x < 0) landmarks.wsPosition.x = size.x;
                                            if (landmarks.wsPosition.x > screenWidth) landmarks.wsPosition.x = screenWidth - size.x;
                                    ...
                                    else if (!sceneview.in2DMode && landmarks.viewState == TaskAtlasData.Scene.Landmark.ViewState.is3D)
                                    {
                                        if (landmarks.wsPosition.z > 0)
                                        {
                                            GUI.Label(new Rect(landmarks.wsPosition.x - 64, landmarks.wsPosition.y, 128, 20), landmarks.title, s);
                    ...
                    for (int t = 0; t < landmarks.tasks.Count; t++)
                    {
                        ...
                        if (scene.showStickies)
                        {
                            if (tasks.isSticky)
                            {
                                ...
                                if (landmarks.viewState == TaskAtlasData.Scene.Landmark.ViewState.is2D && sceneview.in2DMode)
                                {
                                ...
                                else if (landmarks.viewState == TaskAtlasData.Scene.Landmark.ViewState.is3D && !sceneview.in2DMode)
                                {
                                ...
                                if (landmarks.viewState == TaskAtlasData.Scene.Landmark.ViewState.is2D)
                                {
                                    tasks.dist = 0.01f;
                            ...
                            if (tasks.isSticky)
                            {
                                ...
                                if (tasks.isVisible)
                                {
...

                                    {
                                        ...
                                        GUI.DrawTexture(areaRect, Core.tTaskSticky);
                                        #endregion
                                        ...
                                        if (tasks.priority == TaskAtlasData.Scene.Landmark.Task.Priority.urgent)
                                        {
                                        ...
                                        #region TopStats
                                        rPos = new Rect(areaRect.x + Scaled(22), areaRect.y + Scaled(24), Scaled(52), Scaled(24));
                                        GUI.DrawTexture(rPos, Core.tBlue);
                                        rPos = AddRect(areaRect, new Rect(-Scaled(56), Scaled(20), 0, 0));
                                        if (tasks.tStage == null) landmarks.UpdateProgress();
                                        GUI.DrawTexture(rPos, tasks.tStage);


                                        rPos = new Rect(areaRect.x + Scaled(59), areaRect.y + Scaled(24), Scaled(53), Scaled(24));
                                        GUI.DrawTexture(rPos, Core.tTeal);
                                        rPos = AddRect(areaRect, new Rect(-Scaled(8), Scaled(20), 0, 0));
                                        GUI.DrawTexture(rPos, tasks.tPriority);

                                        rPos = new Rect(areaRect.x + Scaled(81), areaRect.y + Scaled(24), Scaled(52), Scaled(24));
                                        GUI.DrawTexture(rPos, Core.tRed);
                                        rPos = AddRect(areaRect, new Rect(Scaled(53), Scaled(20), 0, 0));
                                        ...
                                        ProgressBar(rPos, ((float)tasks.progress / 100), Core.tGreyDark, Core.tGreenP);
                                        #endregion

                                        #region MainText
                                        if (tasks.selectedSubTask == -1)
                                        {
                                            rPos = AddRect(areaRect, new Rect(Scaled(24), Scaled(32), -Scaled(40), -Scaled(54)));
                                            GUI.DrawTexture(rPos, tasks.tTaskName);
                                            rPos = AddRect(areaRect, new Rect(Scaled(24), Scaled(38), -Scaled(40), -Scaled(60)));
                                            GUI.DrawTexture(rPos, tasks.tTaskDescription);
                                        }
                                        else
                                        {
                                            rPos = new Rect(areaRect.x + Scaled(32), areaRect.y + Scaled(37), sWidth - Scaled(48), Scaled(24));
                                            s = new GUIStyle("button");
                                            s.normal.background = Core.tYellow;
                                            ...
                                            if (tasks.selectedSubTask != -1)
                                            {
                                                rPos = new Rect(areaRect.x, areaRect.y + Scaled(34.5f), sWidth, sHeight);
                                                GUI.DrawTexture(rPos, tasks.tTaskNameSmall);
                                        ...
                                        if (tasks.subTasks.Count > 0)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if (prev) s.normal.background = Core.tGreenP;
                                                ...
                                                {
                                                    landmarks.UpdateProgress();
                                                    TaskAtlasEditorWindow.QueueRepaint();
                                                }
                                        ...
                                        bool p = GUI.enabled; Color c = GUI.color;
                                        GUI.enabled = true;
                ...
                #region AtlasMenu
                if (atlas.enabled)
                    if (atlas.state == TaskAtlasData.AtlasState.tasks)
                        GUI.DrawTexture(new Rect(screenWidth - 128, 128 + 64, 128, 240), Core.tGreen);
                GUILayout.BeginArea(new Rect(screenWidth - 128, 128 + 64, 128, 128 * 4));
                {
                    ...
                    if (atlas.enabled)
                    {
                        ...
                        switch (atlas.state)
                        {
                            case TaskAtlasData.AtlasState.landmarks:
                                #region landmarks
                                ...
                                break;
                            #endregion
                            case TaskAtlasData.AtlasState.tasks:
                                #region tasks
                ...
                GUILayout.EndArea();
                if (atlas.enabled && atlas.state == TaskAtlasData.AtlasState.tasks)
                {
                ...
                #region AtlasButton
                GUILayout.BeginArea(new Rect(screenWidth - 128, 128, 128, (128 * 4) + 8));
                {
                ...
                GUILayout.BeginArea(new Rect(screenWidth - 128 - 64, 128, 64, 128));
                {
                    if (atlas.settings)
                    {
                ...
                if (atlas.enabled)
                {
                    ...
                    #region AtlasHandles
                    {
                        ...
                        {
                            case TaskAtlasData.Scene.Landmark.ViewState.is2D: if (sceneview.in2DMode) showLandmark = true; break;
                            case TaskAtlasData.Scene.Landmark.ViewState.is3D: if (!sceneview.in2DMode) showLandmark = true; break;
                            case TaskAtlasData.Scene.Landmark.ViewState.isOrthographic: if (!sceneview.in2DMode && sceneview.orthographic) showLandmark = true; break;
                        }
                        ...
                        if (ws.z > 0 && showLandmark)
                            if (atlas.landmarkSelected == -1)
                            {
                                ...
                                atlas.state = TaskAtlasData.AtlasState.landmarks;
                        ...
                        if (i == atlas.landmarkSelected)
                        {
                            atlas.state = TaskAtlasData.AtlasState.tasks;
                            ...
                            for (int t = 0; t < localDataTasks.Count; t++)
                            {
                                ...
                                if (t == atlas.taskFocus)
                                {
                                    ...
                                    if (GUI.Button(new Rect(ws.x + 4, ws.y + 28, 24, 24), Core.tAtlasGO, GUIStyle.none))
                                    {
                                        ...
                                        scene.atlas.landmarkSelected = -1;
                                        bool is2d = false;
                                        if (landmarks.viewState == TaskAtlasData.Scene.Landmark.ViewState.is2D) is2d = true;
                                ...
                                if (localDataTasks[t].stage != TaskAtlasData.Scene.Landmark.Task.Stage.complete)
                                {
                                    if (localDataTasks[t].priority == TaskAtlasData.Scene.Landmark.Task.Priority.urgent)
                                    {
                                        DrawAlert(ws.x - 8 - 16, ws.y - 8 - 32, 24, 24, Core.tIconExclaim);
                                    }
